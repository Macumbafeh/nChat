...
_G.CHAT_FRAME_TAB_NORMAL_MOUSEOVER_ALPHA = 0.8
...
_G.CHAT_INSTANCE_CHAT_GET = "|Hchannel:INSTANCE_CHAT|h[bg]|h %s:\32"
_G.CHAT_INSTANCE_CHAT_LEADER_GET = "|Hchannel:INSTANCE_CHAT|h[bgl]|h %s:\32"
...
local function ModChat(self)
   ...
   if cfg.showInputBoxAbove then
    ...
    -- Adjust position for combat log tab
    if self == "ChatFrame2" then
        _G[self.."EditBox"]:SetPoint("BOTTOMLEFT", chat, "TOPLEFT", 0, 61) -- Reduce extra spacing
        _G[self.."EditBox"]:SetPoint("BOTTOMRIGHT", chat, "TOPRIGHT", 0, 61)
    else
